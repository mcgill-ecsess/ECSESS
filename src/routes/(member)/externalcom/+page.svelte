<script lang="ts">
	...
	import { externalCommittee } from '$lib/mockData/externalCommittee';
	import { onMount, tick } from 'svelte';
	import { page } from '$app/state';

	let chair = $derived(externalCommittee.filter((member) => member.position.includes('Chair'))[0]);
	let members = $derived(externalCommittee.filter((member) => !member.position.includes('Chair')));
...
<SeoMetaTags
	title="Meet the ECSESS External Committee!"
	description="Meet the team behind ECSESS's sponsorships and external relations."
...
>
	<div class="w-full max-w-360 px-4">
		<h1 class="page-title text-ecsess-50">Meet the External Committee</h1>

		{#if chair}
			<section class="mb-16 w-full">
				<div class="flex w-full items-center gap-4">
					<div class="bg-ecsess-300 h-0.5 flex-1" aria-hidden="true"></div>
					<h2 class="text-ecsess-100 text-xl font-semibold tracking-wider uppercase">Chair</h2>
					<div class="bg-ecsess-300 h-0.5 flex-1" aria-hidden="true"></div>
				</div>
				<div class="mt-4 flex justify-center">
					<CouncilCard
						...
						image={chair.image}
						onViewProfile={() => handleViewProfile(chair)}
						featured
					/>
				</div>
			</section>
		...
		<section class="mt-4 mb-12 text-center">
			<p class="text-ecsess-200 text-lg sm:text-xl">
				Curious to be involved with the External Committee?
				<Link
					href="/join"
