<script lang="ts">
	...
	import { socialCommittee } from '$lib/mockData/socialCommittee';
	import { onMount, tick } from 'svelte';
	import { page } from '$app/state';

	let chair = $derived(socialCommittee.filter((member) => member.position.includes('Chair'))[0]);
	let members = $derived(socialCommittee.filter((member) => !member.position.includes('Chair')));
...
<SeoMetaTags
	title="Meet the ECSESS Social Committee!"
	description="Meet the team behind ECSESS social events."
...
>
	<div class="w-full max-w-360 px-4">
		<h1 class="page-title text-ecsess-50">Meet the Social Committee</h1>
		...
		<section class="mt-4 mb-12 text-center">
			<p class="text-ecsess-200 text-lg sm:text-xl">
				Curious to be involved with the Social Committee?
				<Link
					href="/join"
