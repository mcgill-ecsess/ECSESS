...
import { slide, fade, fly } from 'svelte/transition';
	...
	const steps = [
		{ id: 1, label: 'Partnership Interests' },
		{ id: 2, label: 'Module Selection' },
		{ id: 3, label: 'Engagement Summary' }
	];
	...
	function goToSection(id: 'ecsess' | 'codejam' | 'factory') {
		...
		if (isSectionEnabled(activeSection)) {
			const next = new Set(unlockedSections);
			next.add(activeSection);
			unlockedSections = next;
		}
	...
	const themeMode = $derived.by(() => {
		if (step === 2) return activeSection;
		if (step === 3) return summaryTheme;
		return 'ecsess';
	});
	...
	let unlockedSections = $state(new Set<'ecsess' | 'codejam' | 'factory'>());
	...
	let state = $state({
		...
		selectedModules: {} as Record<string, number>,
	...
	const selectedModules = $derived.by(() =>
		modules.filter((module) => (state.selectedModules[module.id] ?? 0) > 0)
	);
	...
	const ecsessTotal = $derived.by(() => {
		const moduleTotal = selectedModules
			...
			.reduce((sum, module) => sum + module.price * (state.selectedModules[module.id] ?? 0), 0);
		return moduleTotal;
	});
	...
	const canProceed = $derived.by(() => {
		const companyOk = state.sponsorInfo.companyName.trim().length > 0;
		const nameOk = state.sponsorInfo.primaryContactName.trim().length > 0;
		const emailOk = emailValid;
		return companyOk && nameOk && emailOk;
	});
	...
	$effect(() => {
		...
		if (!state.interests.industry) {
			...
			if (unlockedSections.has('ecsess')) {
				const next = new Set(unlockedSections);
				next.delete('ecsess');
				unlockedSections = next;
			}
		...
		if (!state.interests.factory) {
			...
			if (unlockedSections.has('factory')) {
				const next = new Set(unlockedSections);
				next.delete('factory');
				unlockedSections = next;
			}
		...
		if (categoriesToRemove.length > 0) {
			...
			for (const [id, count] of Object.entries(state.selectedModules)) {
				const mod = modules.find((entry) => entry.id === id);
				if (!mod) continue;
				if (!categoriesToRemove.includes(mod.category)) {
					nextSelected[id] = count;
				}
			}
			const prevKeys = Object.keys(state.selectedModules);
			const nextKeys = Object.keys(nextSelected);
			const changed =
				prevKeys.length !== nextKeys.length ||
				prevKeys.some((key) => state.selectedModules[key] !== nextSelected[key]);
			if (changed) {
		...
		if (!state.interests.codejam && state.codejamTierId) {
			state.codejamTierId = '';
			if (unlockedSections.has('codejam')) {
				const next = new Set(unlockedSections);
				next.delete('codejam');
				unlockedSections = next;
			}
		...
		if (step === 2 && !isSectionEnabled(activeSection)) {
	...
	onMount(() => {
		try {
			...
			if (Array.isArray(parsed?.selectedModules)) {
				const counts: Record<string, number> = {};
				parsed.selectedModules.forEach((id: string) => {
					counts[id] = (counts[id] ?? 0) + 1;
				});
				state.selectedModules = counts;
			} else if (parsed?.selectedModules && typeof parsed.selectedModules === 'object') {
				state.selectedModules = parsed.selectedModules;
			}
	...
	function toggleModule(moduleId: string) {
		const current = state.selectedModules[moduleId] ?? 0;
		const next = { ...state.selectedModules };
		if (current > 0) {
	...
	function moduleCount(moduleId: string) {
		return state.selectedModules[moduleId] ?? 0;
	}

	const multiQuantityIds = new Set([
		'networking_workshop',
		'networking_panel',
		'networking_private_event',
		'networking_info_session',
		'factory_workshop_certification'
	]);

	function isMultiQuantity(module: Module) {
		return multiQuantityIds.has(module.id) || module.category === 'media';
	}

	function setModuleCount(moduleId: string, count: number) {
		const next = { ...state.selectedModules };
		if (count <= 0) {
			delete next[moduleId];
		} else {
	...
	function incrementModule(moduleId: string) {
		setModuleCount(moduleId, moduleCount(moduleId) + 1);
	}

	function decrementModule(moduleId: string) {
		setModuleCount(moduleId, moduleCount(moduleId) - 1);
	}
	...
	function confirmRemove() {
		...
		const remainingModuleTotal = modules.reduce((sum, mod) => {
			const count = state.selectedModules[mod.id] ?? 0;
			return sum + mod.price * count;
		}, 0);
		const remainingTotal =
			remainingModuleTotal + (removeTarget.kind === 'codejam' ? 0 : (selectedCodejamTier?.price ?? 0));
		if (step === 3 && remainingTotal === 0) {
			setStep(1);
		}
		removeTarget = null;
	}
	...
	const leftMenuSections = [{ id: 'opportunities', label: 'Opportunities' }] as const;

	const moduleSections = $derived.by(() => {
		const sections: Array<{ id: 'ecsess' | 'codejam' | 'factory'; label: string }> = [];
		if (state.interests.industry && unlockedSections.has('ecsess')) {
			sections.push({ id: 'ecsess', label: 'Mainline Modules' });
		}
		if (state.interests.codejam && unlockedSections.has('codejam')) {
			sections.push({ id: 'codejam', label: 'CodeJam Sponsorship' });
		}
		if (state.interests.factory && unlockedSections.has('factory')) {
			sections.push({ id: 'factory', label: 'The Factory Modules' });
		}
		return sections;
	});
	...
	const summaryTheme = $derived.by(() => {
		const onlyCodejam = state.interests.codejam && !state.interests.industry && !state.interests.factory;
		const onlyFactory = state.interests.factory && !state.interests.industry && !state.interests.codejam;
		if (onlyCodejam) return 'codejam';
		if (onlyFactory) return 'factory';
		return 'ecsess';
	});
...
<Section from="from-ecsess-black" to="to-ecsess-black" via="via-ecsess-800" direction="to-b" contentStart>
	...
	>
		...
		<div class={`grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)_280px] ${step === 1 ? 'mt-2' : 'mt-8'}`}>
			<div class="relative">
				{#if step >= 1 && step <= 3}
					<div
										...
										{#each leftMenuSections as category}
											<button
												...
												class={`w-full rounded-md border px-3 py-2 text-left text-white/80 transition ${
													step === 1 && category.id === 'opportunities'
												...
												{:else}
													{#each ecsessSelected as module}
														...
														<div class="flex snap-start items-start justify-between gap-2 border-b border-transparent last:border-b-0 hover:bg-white/10 hover:border-white/20/60 py-1 rounded-md pr-2 text-left">
															<span class="leading-5 text-left">✓ {module.name}{count > 1 ? ` ×${count}` : ''}</span>
															<button
																type="button"
			...
			<div class="space-y-6">
					...
					>
						...
						<div class="flex items-center justify-center no-print">
							<button
								...
								onclick={() => {
									setStep(2);
									if (unlockedSections.size === 0) {
										const next = new Set(unlockedSections);
										next.add(firstEnabledSection());
										unlockedSections = next;
									}
									goToSection(firstEnabledSection());
								}}
				...
				{#if step === 2}
					...
					>
						...
						<div
							id="section-ecsess"
							class="panel-enter rounded-xl overflow-visible border border-transparent p-0 bg-transparent"
						>
							<div class="mb-8">
								...
								<div class="mt-4 grid gap-6 md:grid-cols-2">
									{#each flagshipModules as module, idx (module.id)}
										{@const isSelected = moduleCount(module.id) > 0}
											...
											>
												...
												>
													...
													<div class="p-4 flex flex-[2] flex-col gap-2">
														...
														{#if isMultiQuantity(module)}
															{#if isSelected}
																<div class="mt-2 flex items-center gap-2">
																	<button
																		type="button"
																		onclick={(e) => { e.stopPropagation(); setModuleCount(module.id, 0); }}
																		class="flex-1 rounded-md border px-3 py-2 text-xs font-semibold text-white transition {actionToneClass(module.category)}"
																	>
																		Remove module from plan
																	</button>
																	<div class="flex items-center gap-2 rounded-md border border-white/20 px-2 py-1 text-xs text-white/80">
																		<button
																			type="button"
																			onclick={(e) => { e.stopPropagation(); decrementModule(module.id); }}
																			class="px-2 py-1 text-white/80 hover:text-white"
																		>
																			−
																		</button>
																		<span class="min-w-[18px] text-center font-semibold text-white">
																			{moduleCount(module.id)}
																		</span>
																		<button
																			type="button"
																			onclick={(e) => { e.stopPropagation(); incrementModule(module.id); }}
																			class="px-2 py-1 text-white/80 hover:text-white"
																		>
																			+
																		</button>
						...
						>
									...
									<div class="mt-4">
											...
											<div class="space-y-4">
												{#each codejamSections as section}
													<div>
														<p class="text-white text-lg font-semibold">{section.title}</p>
														<div class="mt-1 border-t border-white/20"></div>
														<div class="divide-y divide-white/10">
															{#each section.rows as row}
																{@const change = codejamChange(row)}
																{@const shouldAnimate = codejamRowChanged(row)}
																<div class="codejam-row text-sm text-white/90 md:text-[15px]">
							...
							{#if currentCodejamTier}
								{@const tier = currentCodejamTier}
								{@const isSelected = state.codejamTierId === tier.id}
								<div class="codejam-footer-wrap">
									<div class="codejam-footer-inner">
										<button
											type="button"
											onclick={() => setCodejamIndex(Math.max(0, codejamIndex - 1))}
											disabled={codejamIndex === 0}
											class="codejam-footer-btn"
										>
										...
										<button
											...
											disabled={tier.id === 'name_partner' && !data.availability.codejamNamePartnerAvailable}
											class="codejam-footer-btn codejam-footer-select"
										>
				...
				{#if step === 3}
					<section
						id="section-summary"
						class={`space-y-6 rounded-2xl border p-6 shadow-sm no-print theme-panel ${summaryTheme === 'codejam' ? 'theme-codejam' : summaryTheme === 'factory' ? 'theme-factory' : 'theme-ecsess'} ${sectionPanels.center}`}
					>
						<div class="no-print">
										...
										<div class="mt-3 space-y-2">
											{#each ecsessSelected as module}
												...
												<div class="flex items-start justify-between gap-2 rounded-md border border-transparent px-2 py-2 hover:bg-white/10 hover:border-white/20/60">
													<span class="leading-6 text-left">✓ {module.name}{count > 1 ? ` ×${count}` : ''}</span>
													<button
														type="button"
							...
							<div class="mt-8 border-t border-white/15 pt-6">
								<h3 class="text-white text-lg font-semibold text-center">Contact Details</h3>
								<div class="mt-4 grid gap-4 md:grid-cols-2">
									<label class="flex flex-col gap-2 text-sm text-white/80">
										<span class="inline-flex items-center gap-1 whitespace-nowrap">
											Company Name <span class="text-white/60">*</span>
										</span>
										<input
											class="theme-input rounded-md border px-3 py-2"
									...
									<label class="flex flex-col gap-2 text-sm text-white/80">
										<span class="inline-flex items-center gap-1 whitespace-nowrap">
											Primary Contact Name <span class="text-white/60">*</span>
										</span>
										<input
											class="theme-input rounded-md border px-3 py-2"
									...
									<label class="flex flex-col gap-2 text-sm text-white/80">
										<span class="inline-flex items-center gap-1 whitespace-nowrap">
											Primary Contact Email <span class="text-white/60">*</span>
										</span>
										<input
											class="theme-input rounded-md border px-3 py-2"
							...
							<div class="mt-6 flex flex-col items-center gap-3">
									<button
										...
										class="theme-button rounded-md px-8 py-3 text-base font-semibold text-white md:px-10 md:text-lg disabled:opacity-50 disabled:cursor-not-allowed"
										disabled={validationStatus === 'loading' || !canProceed}
									>
										{validationStatus === 'loading'
											? 'Validating...'
						...
						<div class="border-t border-white/15 pt-4 pb-4 space-y-2 text-sm text-white/80">
							...
							{#if step === 2}
								...
								{#if activeSection === 'ecsess' || activeSection === 'factory'}
									<div class="mt-4 rounded-2xl p-4 theme-panel {sectionPanels.wrapper} border-none">
										<div class="flex items-center justify-between gap-3 text-sm text-white/80">
											...
											>
												<span class="tier-info__dot">i</span>
											</button>
										</div>
									</div>
								{/if}
...
<style>
	...
	.tier-info:hover .tier-info__dot {
		transform: scale(1.08);
	...
	.theme-input {
		background-color: color-mix(in srgb, var(--theme-panel) 80%, #000);
	...
	.codejam-footer-inner {
		...
		padding: 12px 16px;
		border-radius: 999px;
		background: rgba(33, 66, 146, 0.95);
	...
	.codejam-footer-btn:hover {
		border-color: rgba(255, 255, 255, 0.6);
		color: #ffffff;
		transform: translateY(-1px);
	}

	.codejam-footer-btn:disabled {
		opacity: 0.4;
		cursor: not-allowed;
		transform: none;
	}

	.codejam-footer-select {
		background: rgba(93, 123, 255, 0.85);
	...
	.codejam-footer-tier__name {
		font-size: 13px;
		font-weight: 700;
		letter-spacing: 0.02em;
	}

	.codejam-footer-tier__price {
		font-size: 12px;
		font-weight: 600;
